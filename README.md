...
Одним из таковых является его определение как интеграл \
$$\int_{0}^{1}\frac{4}{1+x^2}dx=4arctg(x)|_0^1=\pi$$ \
\
Численно данный интеграл можно считать разными способами, среди которых реализованные мной методы левых прямоугольников  
$$\int_{0}^{1}\frac{4}{1+x^2}dx\approx\sum_{i=0}^{N}f(x_i)(x_{i+1}-x_i)$$, дающий ошибку порядка O(h) \
